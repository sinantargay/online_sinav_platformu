...
function App() {
  ...
  if(page==='home') return <main className="shell"><Header/><section className="hero"><p className="badge">Marmara GSF Fotoğraf YL</p><h1>Akademik, kaynaklı ve 5 şıklı online sınav platformu</h1><p className="lead">Üyelik, misafir giriş, yönetici paneli, PDF rapor ve kaynaklı soru havuzu aktif.</p><div className="stats"><div><strong>{questions.length}</strong><span>Soru</span></div><div><strong>{user?'Aktif':'Kapalı'}</strong><span>Oturum</span></div><div><strong>PDF</strong><span>Rapor</span></div></div><div className="paymentBox"><h3>{settings.paymentTitle}</h3><p><b>Hesap Sahibi:</b> {settings.accountHolder || 'Henüz girilmedi'}</p><p><b>Banka:</b> {settings.bankName || 'Henüz girilmedi'}</p><p><b>IBAN:</b> {settings.iban || 'Henüz girilmedi'}</p><p>{settings.paymentText}</p></div><div className="actions"><button onClick={user?startExam:()=>setPage('auth')}>Sınava Başla</button><button className="ghost" onClick={()=>setPage('auth')}>Üyelik / Giriş</button><button className="ghost" onClick={()=>setPage('admin')}>Yönetici Paneli</button></div></section></main>;
