...
function App() {
  ...
  const [settings, setSettings] = useState(() => load('photon_settings', { paymentTitle: 'Ödeme Bilgileri', paymentText: 'Ödeme bilgisi yönetici panelinden girilecek.' }));
  ...
  const Header = () => <nav className="nav"><b>PHOTON</b><span>{user ? `${user.name} · ${user.role}` : 'Fotoğraf Yüksek Lisans Hazırlık'}</span><div className="navActions">{user ? <button className="small ghost" onClick={logout}>Çıkış</button> : <button className="small" onClick={() => setPage('auth')}>Giriş / Üyelik</button>}</div></nav>;
  ...
  function guest(){ const u={name:'Misafir Kullanıcı', role:'misafir'}; setUser(u); localStorage.setItem('photon_user', JSON.stringify(u)); setPage('home'); }
  function login(e){ e.preventDefault(); const f=new FormData(e.currentTarget); const role=f.get('adminCode')==='PHOTON2026'?'yönetici':'üye'; const u={name:f.get('name')||'Sinan Targay', email:f.get('email')||'', role}; setUser(u); localStorage.setItem('photon_user', JSON.stringify(u)); setPage('home'); }
  function saveSettings(e){ e.preventDefault(); const f=new FormData(e.currentTarget); const next={paymentTitle:f.get('paymentTitle'), paymentText:f.get('paymentText')}; setSettings(next); localStorage.setItem('photon_settings', JSON.stringify(next)); alert('Ayarlar kaydedildi.'); }
  ...
  const downloadPdf = () => {
    ...
    doc.setTextColor(20,20,20); doc.setFontSize(11); doc.text(`Rapor No: ${reportNo}`,14,52); doc.text(`Aday: ${user?.name || 'Misafir Kullanıcı'}`,14,62); doc.text(`Tarih: ${new Date().toLocaleString('tr-TR')}`,14,72);
    doc.setFontSize(16); doc.text('Genel Sonuç',14,94); doc.setFontSize(12); doc.text(`Toplam Soru: ${examQuestions.length}`,14,108); doc.text(`Doğru: ${correct}`,14,118); doc.text(`Yanlış/Boş: ${examQuestions.length-correct}`,14,128); doc.text(`Başarı: %${success}`,14,138);
    doc.setFillColor(37,99,235); doc.rect(14,146,Math.max(5,success*1.4),8,'F'); doc.setDrawColor(210); doc.rect(14,146,140,8);
    doc.setFontSize(16); doc.text('Yanlış / Boş Sorular',14,172); let y=184;
  ...
  if(page==='auth') return <main className="shell"><Header/><section className="panel"><h1>Üyelik Sistemi</h1><p className="lead">Üye girişi, misafir girişi ve yönetici paneli bu ekrandan açılır.</p><form className="form" onSubmit={login}><input name="name" placeholder="Ad Soyad" defaultValue="Sinan Targay"/><input name="email" placeholder="E-posta"/><input name="password" type="password" placeholder="Şifre"/><input name="adminCode" placeholder="Yönetici kodu (varsa)"/><label className="check"><input type="checkbox" defaultChecked/> Beni hatırla</label><button>Giriş / Kayıt</button></form><button className="ghost" onClick={guest}>Misafir Girişi</button><p className="hint">Demo yönetici kodu: PHOTON2026</p></section></main>;

  if(page==='home') return <main className="shell"><Header/><section className="hero"><p className="badge">Marmara GSF Fotoğraf YL</p><h1>Akademik, kaynaklı ve 5 şıklı online sınav platformu</h1><p className="lead">Üyelik, misafir giriş, yönetici paneli, PDF rapor ve kaynaklı soru havuzu aktif.</p><div className="stats"><div><strong>{questions.length}</strong><span>Soru</span></div><div><strong>{user?'Aktif':'Kapalı'}</strong><span>Oturum</span></div><div><strong>PDF</strong><span>Rapor</span></div></div><div className="actions"><button onClick={user?startExam:()=>setPage('auth')}>Sınava Başla</button><button className="ghost" onClick={()=>setPage('auth')}>Üyelik / Giriş</button><button className="ghost" onClick={()=>setPage('admin')}>Yönetici Paneli</button></div></section></main>;
